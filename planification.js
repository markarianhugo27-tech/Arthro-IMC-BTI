...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function saveData() {
    // Collecte la structure de données actuelle dans le DOM
    const groups = collectData();

    // NOUVEAU CODE FIREBASE : Sauvegarde dans la base de données
    db.ref("project_tasks")
      .set(groups)
      .then(() => {
        // Pas besoin d'alerte, la base de données mettra à jour l'affichage en temps réel
      })
      .catch((error) => {
  ...
  // DANS planification.js (VERSION FINALE et COMPLÈTE)
        ...
        if (data) {
          ...
          data.forEach((groupData) => {
            ...
            groupData.tasks.forEach((taskData) => {
              ...
              setupTaskInteractivity(taskRow);
            });
          });
        } else {
          // Cas où la base de données est vide
          console.log(
            "Base de données Firebase vide. Initialisation de la structure de base."
          );
          initializeEmptyGrid();
          // On sauvegarde la structure initiale pour la mettre dans Firebase
          saveData();
        }

        // --- 3. LOGIQUE DE RATTACHEMENT DES ÉVÉNEMENTS (CRITIQUE) ---

        // Réattacher les événements d'édition aux groupes (Était dans l'ancien bloc if)
        planificationGrid
          .querySelectorAll(".group-container")
          .forEach((container) => {
            setupGroupEditing(
            ...
            // Pour s'assurer que les événements d'interaction (clics) sont bien réactivés sur les tâches
            container.querySelectorAll(".task-row").forEach((row) => {
              // Remarque: La fonction setupTaskInteractivity() appelée plus haut dans la boucle doit déjà gérer la plupart de ceci.
              // Nous le laissons ici pour la complétude si la logique de createNewTaskRow est légère.
  ...
  // 1. SÉCURITÉ : Vérifie le code d'accès
  if (!checkAccessCode()) {
    // Si l'accès est refusé, on masque et on sort (le code a déjà été implémenté)
    return;
  }

  // 2. SYNCHRONISATION : Démarre le chargement et l'écoute en temps réel avec Firebase.
  // TOUTE la logique de reconstruction du DOM est désormais dans cette fonction.
  startRealtimeSync();

  // 3. AFFICHAGE (Le code doit être mis à jour si vous avez implémenté le masquage par défaut)
  // Assurez-vous d'afficher la grille ici si elle est masquée par défaut par CSS
